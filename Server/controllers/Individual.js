...
export const  getShopByCity = async(req,res) => {
    try{
        ...
        const shop = await User.find({city:city,accountType:'Vendor'},{email:0,state:0,lastName:0,password:0,address:0,pincode:0,image:0,createdAt:0,products:0,profile:0,resetpasswordexpires:0,token:0})
        .populate({
                path: "vendorDetails",
...
export const getAllProductsOfUser = async(req,res) =>{
    try{
        const userId = req.user.id;
            ...
            return respond(res,"User all products found",200,true,products);
    } catch(error) {
        console.log(error)
