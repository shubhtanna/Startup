    ...
    try {
      const res = await axios.post(
        ...
        },
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${process.env.REACT_APP_OPENAI_API_KEY}`, // Update to REACT_APP prefix
          },
        }
      );
      console.log("AI Response:", res.data); // Add logging to inspect the response structure
      return res.data.choices[0].text.trim();
    } catch (error) {
      console.error("Error fetching AI response", error);
      return "Sorry, I can't assist with that right now.";
